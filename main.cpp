...
{
	//CDisplayWindow3D  win("Data set preview",608,544);  // size multiple of 16 to easy video codecs.
	...
	try
	{
		...
		CObservation2DRangeScan::TListExclusionAreasWithRanges filterAreas;
		{
			...
			filterAreas[0].second = std::make_pair<double,double>(-1.5,1.0); // [min,max] Z coordinate for removing the points.
		}
...
//		gl_laser_scans["LASER2"] = opengl::CPlanarLaserScan::Create();
...

		{
			...
			view_im1->setViewportPosition(0  ,0.6,  0.4,0.4);  // (from_x,from_y) -> (width_x,height_y), in ratios
		...
		while (CRawlog::getActionObservationPairOrObservation(*m_rawlog_arch, acts,SF,obs,nEntry) && win2.isOpen() )
		{
			...
			if ( mrpt::system::os::kbhit() )
			{
				...
				switch(c)
				{
					...
					case 's':
					case 'S':
						camera.setAzimuthDegrees( camera.getAzimuthDegrees() - 5.0 );
						break;
					case 'd':
					case 'D':
						camera.setAzimuthDegrees( camera.getAzimuthDegrees() + 5.0 );
			...
			if (IS_CLASS(*obs, CObservationGPS) && obs->sensorLabel=="GPS_DELUO")
			{
				...
				else
				{
					...
					{
						decim=0;
						cout << "Processed " << formatTimeInterval( timeDifference(first_tim,o->timestamp) )
						     << " - used " << mrpt::system::getMemoryUsage()/(1024.0*1024.0) << " Mb.\n"
						     << " lat: " << last_coords.lat.getAsString() << " lon: "<< last_coords.lon.getAsString() << endl;
					}
				}
			}
			else
			if (IS_CLASS(*obs,CObservation2DRangeScan))
			{
				...
				{
					//static const CPose3D hok1_pos(0.536,0,0.273,0, DEG2RAD(21.402),DEG2RAD(0.015) );
					static const CPose3D hok1_pos(0.536,0,0.31,0, DEG2RAD(14),DEG2RAD(0.015) );
			...
			if (want_to_refresh_view)
			{
				...
				if (!load_ok)
					cerr << "**WARNING**: Image seem damaged: " << latest_img->imageLeft.getExternalStorageFile() << endl;
				...
				// Put malaga map image:
				{
					...
					//cout << "px: " << px << endl;
					for (int i=0;i<5;i++)
						mod_map.rectangle(px.x-10-i,px.y-10-i,px.x+10+i,px.y+10+i,TColor(1,1,0),1);
				...
				for (map<string,opengl::CPlanarLaserScan::Ptr>::iterator it=gl_laser_scans.begin();it!=gl_laser_scans.end();++it)
				{
					...
					TMapTime2Lasers::iterator itL = mt2l.lower_bound(latest_img->timestamp);

					if (itL!=mt2l.end())
						it->second->setScan( *itL->second );
				}
				...
				if (++decimate_refresh_points>20)
				{
					...
					for (TMapTime2Lasers::const_iterator it=lstLaserScans.begin();it!=lstLaserScans.end();++it)
					{
						bool valid;
						CPose3D P;
					...
					for (map<string,TMapTime2Lasers>::iterator it=lstLaserScansByName.begin();it!=lstLaserScansByName.end();++it)
						it->second.erase(
							it->second.begin(),
				...
				if (valid)
				{
					...
					gl_grid->setPlaneLimits(
						int(cur_veh_pose.x()/GRID_XY_FREQ)*GRID_XY_FREQ-100,
						int(cur_veh_pose.x()/GRID_XY_FREQ)*GRID_XY_FREQ+100,
						int(cur_veh_pose.y()/GRID_XY_FREQ)*GRID_XY_FREQ-100,
						int(cur_veh_pose.y()/GRID_XY_FREQ)*GRID_XY_FREQ+100 );
					gl_grid->setPlaneZcoord(cur_veh_pose.z()-5);

					// Update the camera:
					camera.setPointingAt(cur_veh_pose.x(), cur_veh_pose.y(), cur_veh_pose.z());
				}
				else
				{
